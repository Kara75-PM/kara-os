...
- 반전 앞에서 장을 쪼갠다. **다음 장을 넘기고 싶어지는 게 여기서 나온다.**
- 마지막 장이 저장·공유를 결정한다. 정리 문장이나 **「무엇을 하면 되는지」 안내**를 여기 둔다.
- 8~12장. 인스타는 20장까지 되지만 12장이 넘으면 **보다가 나가는 사람이 확 는다.**
...
| 표지 | 1 | 넘길지 말지 결정 |
| 상황 | 1~2 | 뭘 하는 얘기인지 |
| 전개 | 4~7 | 핵심 내용, 한 장에 하나씩 |
| 정리 | 1 | 저장될 문장. **갈래를 나눈 표**나 순서 요약 |
| 마지막 안내 (CTA) | 1 | 무엇을 하면 되는지 · 왜 하는지 |
...
3. 브라우저에서 저장 버튼을 누르면 card-01.png ~ card-09.png 가 받은 파일 폴더에 떨어진다
   (숫자가 두 자리다 — card-1 이 아니라 card-01)

⚠️ **크롬은 파일을 여러 개 한꺼번에 받으려 하면 「여러 파일 다운로드 허용?」 창을 띄운다.**
   허용을 안 누르면 첫 장만 받고 나머지가 조용히 막히니, 사용자에게 미리 알려준다.
```
...
- **내 표시**를 하나 정한다. 내용에 맞는 것으로 고른다 — 작업 기록이면 **「문제 / 해결」 딱지**, 시간 순서면 **날짜 띠**, 비교면 **좌우로 나누기**
...
- 캡션은 **첫 두 줄만 미리 보이고 나머지는 접힌다.** 그래서 첫 문장을 맨 앞에 둔다
- 스레드 본문을 그대로 붙이지 말고 존댓말로 풀어 다시 쓴다
- **하라는 것은 하나만.** 두 개면 둘 다 안 한다
- 댓글로 링크를 보내는 방식이면, **댓글에 쓸 단어를 짧고 남들이 안 쓸 만한 것으로** 정한다
- 해시태그는 10~15개. **주제어와, 읽었으면 하는 사람들의 직업**을 섞는다
